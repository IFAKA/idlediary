...
describe("generation export profile", () => {
  ...
  it("uses concat plus generated music mixing args", () => {
    ...
    expect(args.join(" ")).toContain("loudnorm=I=-18:TP=-1.5:LRA=11");
    expect(args.join(" ")).toContain("dynaudnorm");
    expect(args.join(" ")).toContain("volume=0.45,asplit=2[clipmix][clipduck]");
    ...
    expect(args.join(" ")).toContain("volume=1.05[musicbase]");
    expect(args.join(" ")).toContain("sidechaincompress=threshold=0.18:ratio=1.35");
